...
This plugin can be used alongside `@svel/cli-plugin-babel`. When used with Babel, this plugin will output ES2015 and delegate the rest to Babel for auto polyfill based on browser targets.
...
- `config.rule('ts').use('babel-loader')` (when used alongside `@svel/cli-plugin-babel`)
