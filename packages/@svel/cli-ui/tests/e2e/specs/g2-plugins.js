describe('Plugins', () => {
  ...
  it('Should add a plugin', () => {
    ...
    // Search
    cy.get('.package-search-item:contains("@svel/cli-plugin-pwa")').should('be.visible')
    cy.get('.instant-search-input input').clear().type('unit-jest')
    cy.get('.package-search-item:contains("@svel/cli-plugin-pwa")').should('be.not.visible')
    cy.get('.package-search-item:contains("@svel/cli-plugin-unit-jest")').should('be.visible')
    cy.get('.instant-search-input input').clear()
    // Install
    cy.get('.package-search-item:contains("@svel/cli-plugin-pwa") [data-testid="name"]').should('be.visible').click()
    cy.get('[data-testid="download-plugin"]:contains("@svel/cli-plugin-pwa")').should('not.have.class', 'disabled').click()
