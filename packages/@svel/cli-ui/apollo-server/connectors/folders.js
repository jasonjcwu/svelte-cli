...
function isDirectory (file) {
  ...
  try {
    ...
    if (process.env.SVELTE_APP_CLI_UI_DEBUG) console.warn(e.message)
  }
  return false
}
...
function isHidden (file) {
  try {
    ...
    if (process.env.SVELTE_APP_CLI_UI_DEBUG) {
...
function isVueProject (file, context) {
  ...
  try {
    ...
    return Object.keys(pkg.devDependencies || {}).includes('@svel/cli-service')
  } catch (e) {
    if (process.env.SVELTE_APP_CLI_UI_DEBUG) {
      console.log(e)
    }
